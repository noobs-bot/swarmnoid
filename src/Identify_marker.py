    ...
    if ids is not None:
        ...
        for i, marker_id in enumerate(ids.flatten()):
            ...
            if len(corners_array) > 0:
                ...
                cv2.polylines(
                    frame, [np.array(corner_tuples)], True, (0, 255, 0), 2)
                ...
                if cell_x < matrix_size[1] and cell_y < matrix_size[0]:
                    ...
                    orientation.append(
                        (marker_id, calculate_orientation(corners_array)))
                    positions.append(
                        (marker_id, centroid, calculate_orientation(corners_array)))
        ...
        if orientation != []:
            ...
            for item in orientation:
                ...
                cv2.putText(frame, f"ID: {
                            item}", (50, 50 + diff), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                diff = diff + 20
